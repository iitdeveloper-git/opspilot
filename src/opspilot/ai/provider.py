...
class AIProvider:
    ...
    def __init__(self, provider: str = "openai", model: str = "gpt-4o-mini", api_key: str | None = None, base_url: str | None = None):
        self.provider = provider.lower()
        self.model = model
        self.api_key = api_key or os.getenv("AI_API_KEY", "")
        self.base_url = base_url or os.getenv("AI_BASE_URL", "http://localhost:11434/v1")
    ...
    async def _call_openai_compatible(self, system_prompt: str, user_prompt: str) -> str:
        url = f"{self.base_url.rstrip('/')}/chat/completions" if self.provider == 'ollama' else "https://api.openai.com/v1/chat/completions"
